 ...
 */
int moverPelotaComp(char cancha[tamY][tamX], pelota *p, int turno) {
    // Determinar el equipo de la computadora (Rojos o Blancos)
    char equipo = (turno % 2 == 0) ? 'R' : 'B';

    // Coordenadas del arco contrario
    int objetivo_x = (equipo == 'R') ? 0 : tamY - 1; // Línea de gol del rival
    int objetivo_y = tamX / 2; // Centro del arco

    // Si la pelota está cerca del arco rival, intenta marcar un gol
    if (abs(p->pos_x - objetivo_x) <= 1 && abs(p->pos_y - objetivo_y) <= 2) {
        cancha[p->pos_x][p->pos_y] = '_';
        p->pos_x = objetivo_x;
        p->pos_y = objetivo_y;
        cancha[p->pos_x][p->pos_y] = '0';
        printf("¡Gol del equipo %c!\n", equipo);
        return 1;
    }

    // Mover la pelota hacia el arco rival
    int nueva_x = p->pos_x + ((objetivo_x > p->pos_x) ? 1 : -1); // Acercarse al arco
    int nueva_y = p->pos_y + ((objetivo_y > p->pos_y) ? 1 : (objetivo_y < p->pos_y) ? -1 : 0); // Ajustar dirección al arco

    // Verificar límites del campo
    if (nueva_x < 0 || nueva_x >= tamY || nueva_y < 0 || nueva_y >= tamX) {
        return 0; // No mover si está fuera del campo
    }
    ...
    p->pos_x = nueva_x;
    p->pos_y = nueva_y;
    printf("La pelota fue movida hacia (%d, %d) por el equipo %c.\n", nueva_x, nueva_y, equipo);
    return 1;
}
